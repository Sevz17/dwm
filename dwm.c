...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isalwaysontop, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow;
	pid_t pid;
	Client *next;
	Client *snext;
	Client *swallowing;
...
struct Pertag {
	...
	int showextrabars[LENGTH(tags) + 1]; /* display extrabar for the current tag */
};
...
{
	m->wy = (m->showbar && m->topbar) || (m->showextrabar && !m->topbar) ? m->my + bh : m->my;
	m->wh = m->mh - (bh * m->showbar) - (bh * m->showextrabar);
